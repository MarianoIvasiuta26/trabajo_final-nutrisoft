...
@section('title', 'Editar Patología')
...
@section('content')
    <div class="card card-dark mt-3">
        <div class="card-header">
            <h5>Editar Patología - {{$patologia->patologia}}</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('gestion-patologias.update', $patologia->id) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label for="patologia" class="form-label">Patología</label>
                    <input type="text" name="patologia" value="{{$patologia->patologia}}" id="patologia" class="form-control" tabindex="2">
                </div>
                <div class="mb-3">
                    <label for="grupo_patologia">Grupo Patología</label>
                    <input type="text" name="grupo_patologia" value="{{$patologia->grupo_patologia}}" id="grupo_patologia" class="form-control" tabindex="3">
                </div>
                {{--<div class="mb-3">
                ...
                <div>
                    <form action="{{ route('gestion-patologias.index') }}">
                        @csrf
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            guardarButtons.forEach(function (button) {
                button.addEventListener('click', function () {
                    // Muestra un SweetAlert de confirmación
                    swalWithBootstrapButtons.fire({
                        title: '¿Estás seguro de editar la patología?',
